...
pca.towers1$active = ifelse(pca.towers1$sitename == 'Council (NGEE Arctic)' |
                            pca.towers1$sitename == 'Council, Alaska',
...
png(filename = './output/special runs/council/base.png',width = 6,height = 6,units = 'in',res = 500)
...
#save the test here
writeRaster(x = base,filename = './output/special runs/chersky/base_all_towers.tif',overwrite = T)

#compare this to the original base image
base.og = stack('./output/active/base_all_towers.tif')

base.dif = base.og - base

#create the base
dif.ster = projectRaster(from = base.dif,crs = proj)

png(filename = './output/special runs/council/difference.png',width = 6,height = 6,units = 'in',res = 500)
ggplot()+theme_bw()+ggtitle('All Active Sites')+
  geom_polygon(data = wm,aes(x = long,y=lat,group=group),
               fill = 'gray80',lwd = 0.25)+
  layer_spatial(dif.ster$layer)+
  geom_point(data = towers.st,aes(x.1,y.1),col='black',fill='green',pch=25,size=2)+
  scale_fill_gradientn('ED',
                       na.value = 'transparent',
                       colours = pal,
...
path = paste('./output/special runs/council/ext/',pca.towers$sitename[ext],'.tif',sep = '')
#save off rasters
...
png(filename = './output/special runs/council/barplot_reduction.png',width = 10,height = 4,units = 'in',res = 2000)
...
write.csv(x = bars,file = './output/special runs/council/meanreduction.csv',row.names = F)


#save off difference maps
path = paste('./output/special runs/council/difs/',pca.towers$sitename[ext],'_dif.tif',sep = '')
#save off rasters
for (i in 1:length(difs)) {
...
#plot
path = paste('./output/special runs/chersky/plots/',pca.towers$sitename[ext],'_dif.png',sep = '')

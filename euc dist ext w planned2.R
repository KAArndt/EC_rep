...
pca.input = pca.input[complete.cases(pca.input[,c(3:23)]),]
...
int = subset(pca.towers,pca.towers$sitename == 'Radisson Ecological Research Station' |
               pca.towers$sitename == 'CEF cluster' | pca.towers$sitename == 'Schefferville')
...
pca.towers1$ch4 = ifelse(pca.towers1$sitename == 'Resolute' |
                           pca.towers1$sitename == 'Kangiqsualujjuaq' |
                           pca.towers1$sitename == 'Pond Inlet' |
...
pca.towers1$annual = ifelse(pca.towers1$sitename == 'Resolute' |
                              pca.towers1$sitename == 'Kangiqsualujjuaq' |
                              pca.towers1$sitename == 'Pond Inlet' |
                              pca.towers1$sitename == 'Yessey' |
                              pca.towers1$sitename == 'Kular' |
...
png(filename = './output/may/active/w_planned/base.png',width = 6,height = 6,units = 'in',res = 500)
...
#save the base here
writeRaster(x = base,filename = './output/may/active/w_planned/base_all_towers.tif',overwrite = T)
...
path = paste('./output/may/active/w_planned/ext/',pca.towers$sitename[ext],'.tif',sep = '')
#save off rasters
...
path = paste('./output/may/active/w_planned/difs/',pca.towers$sitename[ext],'_dif.tif',sep = '')
#save off rasters
...
png(filename = './output/may/active/w_planned/barplot_reduction.png',width = 10,height = 4,units = 'in',res = 2000)
...
write.csv(x = bars,file = './output/may/active/w_planned/meanreduction.csv',row.names = F)
...
#plot
path = paste('./output/may/active/w_planned/plots/',pca.towers$sitename[ext],'_dif.png',sep = '')
...
####################################################################################
#the ch4 network
net = which(pca.towers1$ch4 == 'CH4' & pca.towers1$active == 'active')
ext = which(pca.towers1$ch4 != 'CH4' & pca.towers1$active == 'active')
...
#net
towers = SpatialPointsDataFrame(coords = pca.towers[net,c(1,2)],
                                data = pca.towers[net,],proj4string = crs(base))
towers = spTransform(x = towers,CRSobj = proj)
towers.net = as.data.frame(towers)

#ext
towers = SpatialPointsDataFrame(coords = pca.towers[ext,c(1,2)],
                                data = pca.towers[ext,],proj4string = crs(base))
towers = spTransform(x = towers,CRSobj = proj)
towers.ext = as.data.frame(towers)

base.ster = projectRaster(from = base,crs = proj)

png(filename = './output/may/ch4/w_planned/base.png',width = 6,height = 6,units = 'in',res = 500)
ggplot()+theme_bw()+ggtitle('CH4 Sites')+
  geom_polygon(data = wm,aes(x = long,y=lat,group=group),
               fill = 'gray80',lwd = 0.25)+
  layer_spatial(base.ster$base.dist)+
  geom_point(data = towers.net,aes(x.1,y.1),col='black',fill='green',pch=25,size=2)+
  geom_point(data = towers.ext,aes(x.1,y.1),col='green',fill='transparent',pch=25,size=2)+
  scale_fill_gradientn('ED',
                       na.value = 'transparent',
                       colours = pal,
                       #trans = 'log',
...
#save the base here
writeRaster(x = base,filename = './output/may/ch4/w_planned/base_2_towers.tif',overwrite = T)
...
#create a path of file names
path = paste('./output/may/ch4/w_planned/ext/',pca.towers$sitename[ext],'.tif',sep = '')
#save off rasters
for (i in 1:length(dist.rasts)) {
  writeRaster(x = dist.rasts[[i]],filename = path[i],overwrite=T)
  progress(i,length(dist.rasts))
...
#save off difference maps
path = paste('./output/may/ch4/w_planned/dif/',pca.towers$sitename[ext],'_dif.tif',sep = '')
#save off rasters
for (i in 1:length(difs)) {
  writeRaster(x = difs[[i]],filename = path[i],overwrite=T)
  progress(i,length(difs))
...
#plot
path = paste('./output/may/ch4/w_planned/plots/',pca.towers$sitename[ext],'_dif.png',sep = '')
...
png(filename = './output/may/ch4/w_planned/barplot_reduction.png',width = 10,height = 4,units = 'in',res = 2000)
ggplot(data = top)+theme_bw()+ggtitle('Top 50 Percentile Improvements')+
...
png(filename = './output/may/ch4/w_planned/barplot_reduction_country.png',width = 10,height = 4,units = 'in',res = 2000)
ggplot(data = top)+theme_bw()+ggtitle('Top 50 Percentile Improvements')+
  geom_bar(aes(reorder(sitename, -means*-1),means*-1,fill=country),stat = 'identity')+
  scale_y_continuous(expand = c(0,0),limits = c(0,upper.limit),'Mean ED Reduction')+
  scale_x_discrete('Site')+
...
write.csv(x = bars,file = './output/may/ch4/w_planned/meanreduction.csv',row.names = F)

####################################################################################
#Winter NETWORK #########################################################
####################################################################################
#the winter network
net = which(pca.towers1$annual == 'annual' & pca.towers1$active == 'active')
ext = which(pca.towers1$annual != 'annual' & pca.towers1$active == 'active')
...
png(filename = './output/may/annual/w_planned/base.png',width = 6,height = 6,units = 'in',res = 500)
...
#save the base here
writeRaster(x = base,filename = './output/may/annual/w_planned/base_2_towers.tif',overwrite = T)
...
#create a path of file names
path = paste('./output/may/annual/w_planned/ext/',pca.towers$sitename[ext],'.tif',sep = '')
#save off rasters
for (i in 1:length(dist.rasts)) {
  writeRaster(x = dist.rasts[[i]],filename = path[i],overwrite=T)
  progress(i,length(dist.rasts))
...
#save off difference maps
path = paste('./output/may/annual/w_planned/dif/',pca.towers$sitename[ext],'_dif.tif',sep = '')
#save off rasters
for (i in 1:length(difs)) {
  writeRaster(x = difs[[i]],filename = path[i],overwrite=T)
  progress(i,length(difs))
...
#plot
path = paste('./output/may/annual/w_planned/plots/',pca.towers$sitename[ext],'_dif.png',sep = '')
...
png(filename = './output/may/annual/w_planned/barplot_reduction.png',width = 10,height = 4,units = 'in',res = 2000)
...
png(filename = './output/may/annual/w_planned/barplot_reduction_country.png',width = 10,height = 4,units = 'in',res = 2000)
ggplot(data = top)+theme_bw()+ggtitle('Top 50 Percentile Improvements - Annual')+
  geom_bar(aes(reorder(sitename, -means*-1),means*-1,fill=country),stat = 'identity')+
  scale_y_continuous(expand = c(0,0),limits = c(0,-1*min(top$means)+0.005),'Mean ED Reduction')+
  scale_x_discrete('Site')+
...
write.csv(x = bars,file = './output/may/annual/w_planned/meanreduction.csv',row.names = F)

####################################################################################
#Winter CH4 NETWORK #########################################################
####################################################################################
#the winter and ch4 network
net = which(pca.towers1$active == 'active' & pca.towers1$ch4 == 'CH4' & pca.towers1$annual == 'annual')
ext = which((pca.towers1$ch4 != 'CH4' | pca.towers1$annual != 'annual') & pca.towers1$active == 'active')
...
png(filename = './output/may/annual ch4/w_planned/base.png',width = 6,height = 6,units = 'in',res = 500)
...
#save the base here
writeRaster(x = base,filename = './output/may/annual ch4/w_planned/base_2_towers.tif',overwrite = T)
...
#create a path of file names
path = paste('./output/may/annual ch4/w_planned/ext/',pca.towers$sitename[ext],'.tif',sep = '')
#save off rasters
for (i in 1:length(dist.rasts)) {
  writeRaster(x = dist.rasts[[i]],filename = path[i],overwrite=T)
  progress(i,length(dist.rasts))
...
#save off difference maps
path = paste('./output/may/annual ch4/w_planned/dif/',pca.towers$sitename[ext],'_dif.tif',sep = '')
#save off rasters
for (i in 1:length(difs)) {
  writeRaster(x = difs[[i]],filename = path[i],overwrite=T)
  progress(i,length(difs))
...
#plot
path = paste('./output/may/annual ch4/w_planned/plots/',pca.towers$sitename[ext],'_dif.png',sep = '')
...
png(filename = './output/may/annual ch4/w_planned/barplot_reduction.png',width = 10,height = 4,units = 'in',res = 2000)
...
png(filename = './output/may/annual ch4/w_planned/barplot_reduction_country.png',width = 10,height = 4,units = 'in',res = 2000)
ggplot(data = top)+theme_bw()+ggtitle(expression('Top 25 Percentile Improvements - Annual'~CH[4]))+
  geom_bar(aes(reorder(sitename, -means*-1),means*-1,fill=country),stat = 'identity')+
...
write.csv(x = bars,file = './output/may/annual ch4/w_planned/meanreduction.csv',row.names = F)

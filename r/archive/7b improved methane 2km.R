...
net = which(pca.towers$active == 'active' & pca.towers$methane == 'methane')
...
impch4.dist = numeric(length = nrow(euci.net))
{orig = Sys.time() #start the clock for timing the process
  for (i in 1:nrow(euci.net)) {
    impch4.dist[i] = mean(euci.net[i,topn(vec = euci.net[i,],n = num,decreasing = F,hasna = F)])
#    improve.dist[i] = min(euci.net[i,])
  }
  Sys.time() - orig} #stop the clock

#make the base image
impdf = data.frame(df$x,df$y,impch4.dist)
...
#save the imp here
writeRaster(x = imp,filename = './output/impch4_2kmv2.tif',overwrite = T)

#######################################################################################
imp = rast('./output/impch4_2kmv2.tif')
base = rast('./output/methane_2kmv2.tif')
...
new = subset(pca.towers,pca.towers$active == 'active' & pca.towers$methane == 'methane')
old = subset(pca.towers,complete.cases(pca.towers$`2022 list`) & pca.towers$active == 'active' & pca.towers$Start_CO2 < 2022 & pca.towers$methane == 'methane')

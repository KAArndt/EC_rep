...
#Single Site Rep ########################################################### ###################
euci = read_rds(file = './euclidean_distance_matrix/euci_2km.rds')
ykdb = euci[,657]
...
pal = hcl.colors(n = 9,palette = 'Mako',rev = T)
pal = pal[-c(4,6)]
...
ch4  = rast('./output/methane_2km.tif')
year = rast('./output/annual_2km.tif')
ach4 = rast('./output/annual_methane_2km.tif')

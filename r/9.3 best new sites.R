...
#ranking of sites
ranks = read.csv(file = './output/reductions/meanreduction_remaining_mean_2_khentii.csv')
...
tower.data$active  = ifelse(tower.data$site == "Yessey" |
                            ...
                            tower.data$site == 'Chulman','active',tower.data$active)
...
#save the base here
writeRaster(x = base,filename = './output/improved_network/next_five_sites/improved_base_2kmv2_mean_chulman_3.tif',overwrite = T)
...
#saveRDS(object = eucis,file = './euclidean_distance_matrix/ext_eucis_2km_mean_3_chulman.rds')
#eucis = read_rds(file = './euclidean_distance_matrix/ext_eucis_2km_mean_3_chulman.rds')
...
# path = paste('./output/remaining_ext/3_chulman/',tower.data$site[ext],'.tif',sep = '')
# #save off rasters
...
#load in the base
base = rast('./output/improved_network/next_five_sites/improved_base_2kmv2_mean_chulman_3.tif')
...
write.csv(x = bars,file = './output/reductions/meanreduction_remaining_mean_3_chulman.csv',row.names = F)
...
# for (i in 1:length(dif.ag)) {
#   png(filename = paste('./output/remaining_difs/3_chulman/',extention.towers$site[i],'.png',sep = ''),width = 4,height = 4,units = 'in',res = 100)
#   print(plot_list[[i]])
#   dev.off()
#   progress(value = i,max.value = length(dif.ag))
# }

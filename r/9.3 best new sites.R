...
tower.data = fread('./data/next_5_sites/base_2.csv')

#First addition ############################################################################################
tower.data$order = seq(1,length(tower.data$MeanTemp)) #important for merging back the tower order

#ranking of sites
ranks = read.csv(file = './output/reductions/meanreduction_remaining_2.csv')
...
write_csv(x = tower.data,'./data/next_5_sites/base_3.csv')
...
orig = rast('./output/improved_network/next_five_sites/improved_base_2km_2.tif')

plot(base,range=c(0,4.5))
points(towers,col='red')

new = subset(towers,towers$site == name)
plot(orig - base,range=c(0.01,1))
points(new,col='red')

#save the base here
writeRaster(x = base,filename = './output/improved_network/next_five_sites/improved_base_2km_3.tif',overwrite = T)
...
write.csv(x = bars,file = './output/reductions/meanreduction_remaining_3.csv',row.names = F)

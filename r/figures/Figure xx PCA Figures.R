...
srt = srt[complete.cases(srt$Permafrost),]
srt = srt[complete.cases(srt$SWIRaug),]
srt = srt[complete.cases(srt$Silt),]
srt = srt[complete.cases(srt$NDVImax),]
...
pca.ta = subset(pca.t,pca.t$active.2024 == 'active')
pca.ex = subset(pca.t,pca.t$active.2024 != 'active' | is.na(pca.t$active))
...
#PCA raster figure
png('./figures/Figure xx PCA raster.png',width = 5,height = 4,units = 'in',res = 1500)
plot(r,axes=F)
dev.off()


library(ggplot2)
library(ggspatial)
library(terra)
library(sf)
library(data.table)
library(cowplot)
library(ggnewscale)
library(RColorBrewer)

#######################################################################################
base             = rast('./output/base_network/base_2km.tif')
methane          = rast('./output/base_network/methane_2km.tif')
annual           = rast('./output/base_network/annual_2km.tif')
annual.methane   = rast('./output/base_network/annual_methane_2km.tif')

base.i           = rast('./output/improved_network/improved_base_2km.tif')
methane.i        = rast('./output/improved_network/improved_methane_2km.tif')
annual.i         = rast('./output/improved_network/improved_annual_2km.tif')
annual.methane.i = rast('./output/improved_network/improved_annual_methane_2km.tif')

#base = base/minmax(base)[2] #use this to re-scale from 0-1

#world map for plotting
sf_use_s2(FALSE) #need to run this before next line
countries = rnaturalearth::ne_countries(returnclass = "sf") %>%
  st_crop(y = st_bbox(c(xmin = -180, ymin = 44, xmax = 180, ymax = 90))) %>%
  smoothr::densify(max_distance = 1) %>%
  st_transform(crs(base))

#create aggregates for the plots
base.ag           = aggregate(x = base,fact = 4,fun = mean,na.rm = T)
methane.ag        = aggregate(x = methane,fact = 4,fun = mean,na.rm = T)
annual.ag         = aggregate(x = annual,fact = 4,fun = mean,na.rm = T)
annual.methane.ag = aggregate(x = annual.methane,fact = 4,fun = mean,na.rm = T)

base.ag.i           = aggregate(x = base.i,fact = 4,fun = mean,na.rm = T)
methane.ag.i        = aggregate(x = methane.i,fact = 4,fun = mean,na.rm = T)
annual.ag.i         = aggregate(x = annual.i,fact = 4,fun = mean,na.rm = T)
annual.methane.ag.i = aggregate(x = annual.methane.i,fact = 4,fun = mean,na.rm = T)



#create a scatter to show data spread
# base.df = as.data.frame(base)
# 
# ggplot()+theme_classic()+
#   geom_density(aes(base.df),fill='gray',alpha=0.5)+
#   scale_x_continuous(expand = c(0,0),'Euc. Dist.')+
#   scale_y_continuous(expand = c(0,0),limits = c(0,1))+
#   theme(text = element_text(size = 8))

#base network towers
#load in extracted site data from extraction codes
tower.data = fread(file = './data/final.tower.data.csv')

base.towers            = subset(tower.data,tower.data$active.2022 == 'active')

methane.towers         = subset(tower.data,tower.data$active.2022 == 'active' &
                                  tower.data$methane.2022 == 'methane')

annual.towers          = subset(tower.data,tower.data$active.2022 == 'active' &
                                  tower.data$Season_Activity.2022 == 'All year')

annual.methane.towers  = subset(tower.data,tower.data$active.2022 == 'active' &
                                  tower.data$annualmethane2022 == 'annualmethane')

#improved network
base.towers.i            = subset(tower.data,tower.data$active.2024 == 'active')

methane.towers.i         = subset(tower.data,tower.data$active.2024 == 'active' &
                                    tower.data$methane.2024 == 'methane')

annual.towers.i          = subset(tower.data,tower.data$active.2024 == 'active' &
                                    tower.data$Season_Activity.2024 == 'All year')

annual.methane.towers.i  = subset(tower.data,tower.data$active.2024 == 'active' &
                                    tower.data$annualmethane2024 == 'annualmethane')



#color pallette
pal = hcl.colors(n = 9,palette = 'Vik')
pal = pal[-c(4,6)]
#pal = c('#FEEDB9','#E88D7A','#72509A','#8AABD6','#F2F7FB')

################################################################################################################
#   PAPER PLOT

#base
base.plot = ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(base.ag)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = base.towers,aes(x,y,pch=Season_Activity.2022,fill=methane.2022),col='black',show.legend = F,cex = 0.8)+
  scale_shape_manual(values = c(21,24),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 5),
        axis.title = element_blank(),
        legend.position = 'none')+
  annotate(geom = 'text',x = -3403909,y = 3474170,label = expression('Growing Season'~CO[2]),size=2)
#base.plot

#methane
methane.plot = ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(methane.ag)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = methane.towers,aes(x,y,fill=methane.2022,pch=Season_Activity.2022),col='black',show.legend = F,cex = 0.8)+
  scale_shape_manual(values = c(21,24),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 8),
        axis.title = element_blank(),
        legend.position = 'none')+
  annotate(geom = 'text',x = -3403909,y = 3474170,label = expression('Growing Season'~CH[4]),size=2)
#methane.plot

#annual
annual.plot = ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(annual.ag)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = annual.towers,aes(x,y,fill=methane.2022,pch=Season_Activity.2022),col='black',show.legend = F,cex = 0.8)+
  scale_shape_manual(values = c(21,2),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 8),
        axis.title = element_blank(),
        legend.position = 'none')+
  annotate(geom = 'text',x = -3403909,y = 3474170,label = expression('Year-Round'~CO[2]),size=2)
#annual.plot

#annual methane
annual.methane.plot = ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(annual.methane.ag)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = annual.methane.towers,aes(x,y,fill=methane.2022,pch=Season_Activity.2022),col='black',show.legend = F,cex = 0.8)+
  scale_shape_manual(values = c(21,2),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 8),
        axis.title = element_blank(),
        legend.position = 'none')+
  annotate(geom = 'text',x = -3403909,y = 3474170,label = expression('Year-Round'~CH[4]),size=2)
#annual.methane.plot
####################################################################################################


#base.i
base.plot.i = ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(base.ag.i)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = base.towers.i,aes(x,y,fill=methane.2024,pch=Season_Activity.2024,col=methane.2024),
             col='black',show.legend = F,cex = 0.8)+
  scale_shape_manual(values = c(21,24),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 5),
        axis.title = element_blank(),
        legend.position = 'none')+
  annotate(geom = 'text',x = -3403909,y = 3474170,label = expression('Growing Season'~CO[2]),size=2)
#base.plot

#methane
methane.plot.i = ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(methane.ag.i)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = methane.towers.i,aes(x,y,fill=methane.2024,pch=Season_Activity.2024),col='black',show.legend = F,cex = 0.8)+
  scale_shape_manual(values = c(21,24),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 8),
        axis.title = element_blank(),
        legend.position = 'none')+
  annotate(geom = 'text',x = -3403909,y = 3474170,label = expression('Growing Season'~CH[4]),size=2)
#methane.plot

#annual
annual.plot.i = ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(annual.ag.i)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = annual.towers.i,aes(x,y,fill=methane.2024,pch=Season_Activity.2024),col='black',show.legend = F,cex = 0.8)+
  scale_shape_manual(values = c(21,2),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 8),
        axis.title = element_blank(),
        legend.position = 'none')+
  annotate(geom = 'text',x = -3403909,y = 3474170,label = expression('Year-Round'~CO[2]),size=2)
#annual.plot

#annual methane
annual.methane.plot.i = ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(annual.methane.ag.i)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','ER4','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = annual.methane.towers.i,aes(x,y,fill=methane.2024,pch=Season_Activity.2024),col='black',show.legend = F,cex = 0.8)+
  scale_shape_manual(values = c(21,2),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 8),
        axis.title = element_blank(),
        legend.key.height = unit(x = 0.04,units = 'in'),
        legend.key.width = unit(x = 0.18,units = 'in'),
        legend.direction = 'horizontal',
        legend.text = element_text(size = 6),
        legend.title = element_text(size = 6.5),
        legend.position = c(0.03,0.05),
        legend.title.position = 'top')+
  annotate(geom = 'text',x = -3403909,y = 3474170,label = expression('Year-Round'~CH[4]),size=2)
#annual.methane.plot.i
###################################################################################################

#plot all 8 together
png(filename = './figures/figure 1 all.4.scenarios.png',width = 6,height = 9.5,units = 'in',res = 2000)
plot_grid(base.plot,base.plot.i,
          methane.plot,methane.plot.i,
          annual.plot,annual.plot.i,
          annual.methane.plot,annual.methane.plot.i,
          labels = c('a','e',
                     'b','f',
                     'c','g',
                     'd','h'),label_size = 7,rows = 4)
dev.off()

#plot all 4 together
png(filename = './figures/all.4.scenarios_2024.png',width = 6,height = 5,units = 'in',res = 2000)
plot_grid(base.plot.i,
          methane.plot.i,
          annual.plot.i,
          annual.methane.plot.i,
          labels = c('a',
                     'b',
                     'c',
                     'd'),label_size = 7,rows = 2)
dev.off()

#plot all 4 together
png(filename = './figures/all.4.scenarios_2022.png',width = 6,height = 5,units = 'in',res = 2000)
plot_grid(base.plot,
          methane.plot,
          annual.plot,
          annual.methane.plot,
          labels = c('a',
                     'b',
                     'c',
                     'd'),label_size = 7,rows = 2)
dev.off()

#presentation size #####################################################################################################
#base.i
base.plot.i = ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(base.ag.i)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = base.towers.i,aes(x,y,fill=methane,pch=Season_Activity,col=methane),
             col='black',show.legend = F,cex = 1.2)+
  scale_shape_manual(values = c(21,24),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 5),
        axis.title = element_blank(),
        legend.position = 'none')+
  annotate(geom = 'text',x = -3454909,y = 3895170,label = expression('Growing Season'~CO[2]),size=3)
#base.plot

#methane
methane.plot.i = ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(methane.ag.i)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = methane.towers.i,aes(x,y,fill=methane,pch=Season_Activity),col='black',show.legend = F,cex = 1.2)+
  scale_shape_manual(values = c(21,24),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 8),
        axis.title = element_blank(),
        legend.position = 'none')+
  annotate(geom = 'text',x = -3454909,y = 3895170,label = expression('Growing Season'~CH[4]),size=3)
#methane.plot

#annual
annual.plot.i = ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(annual.ag.i)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = annual.towers.i,aes(x,y,fill=methane,pch=Season_Activity),col='black',show.legend = F,cex = 1.2)+
  scale_shape_manual(values = c(21,2),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 8),
        axis.title = element_blank(),
        legend.position = 'none')+
  annotate(geom = 'text',x = -3454909,y = 3895170,label = expression('Year-Round'~CO[2]),size=3)
#annual.plot

#annual methane
annual.methane.plot.i = ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(annual.ag.i)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = annual.methane.towers.i,aes(x,y,fill=methane,pch=Season_Activity),col='black',show.legend = F,cex = 1.2)+
  scale_shape_manual(values = c(21,2),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 8),
        axis.title = element_blank(),
        legend.key.height = unit(x = 0.04,units = 'in'),
        legend.key.width = unit(x = 0.18,units = 'in'),
        legend.direction = 'horizontal',
        legend.text = element_text(size = 8),
        legend.title = element_text(size = 8.5),
        legend.position = c(0.03,0.05),
        legend.title.position = 'top')+
  annotate(geom = 'text',x = -3454909,y = 3895170,label = expression('Year-Round'~CH[4]),size=3)
#annual.methane.plot.i
###################################################################################################

#plot all 8 together
# png(filename = './figures/figure 1 all.4.scenarios.png',width = 6,height = 9.5,units = 'in',res = 2000)
# plot_grid(base.plot,base.plot.i,
#           methane.plot,methane.plot.i,
#           annual.plot,annual.plot.i,
#           annual.methane.plot,annual.methane.plot.i,
#           labels = c('a','e',
#                      'b','f',
#                      'c','g',
#                      'd','h'),label_size = 7,rows = 4)
# dev.off()

#plot all 4 together
png(filename = './figures/all.4.scenarios_presentation.png',width = 6,height = 5,units = 'in',res = 2000)
plot_grid(base.plot.i,
          methane.plot.i,
          annual.plot.i,
          annual.methane.plot.i,
          labels = c('a',
                     'b',
                     'c',
                     'd'),label_size = 7,rows = 2)
dev.off()

#play with single ones #############################################################################
#base
ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(base.ag)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = base.towers,aes(x,y,pch=Season_Activity,fill=methane),
             col='black',show.legend = F,cex = 2)+
  scale_shape_manual(values = c(21,24),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 18),
        axis.title = element_blank(),
        legend.position = c(0.05,0.05),
        legend.direction = 'horizontal',
        legend.title.position = 'top',
        legend.key.width = unit(0.5,'in'))+
  annotate(geom = 'text',x = -3403909,y = 3474170,label = expression('2022 Growing Season'~CO[2]),size=5)


#base i
ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(base.ag.i)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = base.towers.i,aes(x,y,pch=Season_Activity,fill=methane),
             col='black',show.legend = F,cex = 2)+
  scale_shape_manual(values = c(21,24),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(text = element_text(size = 18),
        axis.title = element_blank(),
        legend.position = c(0.05,0.05),
        legend.direction = 'horizontal',
        legend.title.position = 'top',
        legend.key.width = unit(0.5,'in'))+
  annotate(geom = 'text',x = -3403909,y = 3474170,label = expression('2024 Growing Season'~CO[2]),size=5)



# map for dani ####################################################################################################
oursites = subset(base.towers.i,
                  base.towers.i$site == 'Resolute Bay' |
                    base.towers.i$site == 'Pond Inlet (PP)' |
                    base.towers.i$site == 'Iqaluit (PP)' |
                    base.towers.i$site == 'Council (Permafrost Pathways)' |
                    base.towers.i$site == 'Churchill Fen' |
                    base.towers.i$site == 'Yukon-Kuskokwim Delta, Izaviknek-Kingaglia uplands, Burned 2015' |
                    base.towers.i$site == 'Yukon-Kuskokwim Delta, Izaviknek-Kingaglia uplands, Unburned')


png(filename = './figures/PPsitesmap.png',width = 8,height = 8,units = 'in',res = 2000)
ggplot()+theme_map()+
  geom_sf(data = countries,fill='gray',col='gray40')+
  layer_spatial(base.ag.i)+
  scale_fill_gradientn('Representativeness',
                       na.value = 'transparent',
                       colours = pal,
                       limits = c(0,1.77*2),
                       breaks = c(0,1.77,1.77*2),
                       labels = c('Good','Cutoff','Poor'),
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = base.towers.i,aes(x,y,pch=Season_Activity,fill=methane),
             col='black',show.legend = F,cex = 2,alpha=0.8)+
  scale_shape_manual(values = c(21,24),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  geom_point(data = oursites,aes(x,y),pch=21,fill='yellow1',col='black',cex = 3.5)+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
  theme(legend.text = element_text(size = 16),
        axis.title = element_blank(),
        legend.position = c(0.05,0.06),
        legend.direction = 'horizontal',
        legend.title.position = 'top',
        legend.key.width = unit(0.45,'in'))
dev.off()

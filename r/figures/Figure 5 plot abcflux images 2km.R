...
base.towers           = subset(abcf,as.numeric(abcf$end_year) >= 2022 & abcf$active.2024 == 'active')
methane.towers        = subset(abcf,as.numeric(abcf$end_year) >= 2022 & abcf$active.2024 == 'active' & abcf$methane.2024 == 'methane')
annual.towers         = subset(abcf,as.numeric(abcf$end_year) >= 2022 & abcf$active.2024 == 'active' & abcf$Season_Activity.2024 == 'All year')
annual.methane.towers = subset(abcf,as.numeric(abcf$end_year) >= 2022 & abcf$active.2024 == 'active' & abcf$annualmethane2024 == 'annualmethane')
...
base.plot = ggplot()+theme_map()+
  ...
  scale_fill_gradientn('Representativeness',
                       ...
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = base.towers,aes(x,y,pch=Season_Activity.2024,fill=methane.2024),col='black',show.legend = F,cex = 0.8)+
...
methane.plot = ggplot()+theme_map()+
  ...
  scale_fill_gradientn('Representativeness',
                       ...
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = methane.towers,aes(x,y,fill=methane.2024,pch=Season_Activity.2024),col='black',show.legend = F,cex = 0.8)+
  scale_shape_manual(values = c(21,24),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
...
annual.plot = ggplot()+theme_map()+
  ...
  scale_fill_gradientn('Representativeness',
                       ...
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = annual.towers,aes(x,y,fill=methane.2024,pch=Season_Activity.2024),col='black',show.legend = F,cex = 0.8)+
...
annual.methane.plot = ggplot()+theme_map()+
  ...
  scale_fill_gradientn('Representativeness',
                       ...
                       oob = scales::squish)+  
  new_scale("fill") +
  geom_point(data = annual.methane.towers,aes(x,y,fill=methane.2024,pch=Season_Activity.2024),col='black',show.legend = F,cex = 0.8)+
  scale_shape_manual(values = c(21,2),'Annual Cover',labels = c('Annual','Not Annual'))+
  scale_fill_manual(values = c('red','green3'))+
  scale_x_continuous(limits = c(-5093909,4542996))+
  scale_y_continuous(limits = c(-3687122,4374170))+
...
#plot all 4 together
png(filename = './figures/figure 5 all.4.scenarios_abc.png',width = 6,height = 5,units = 'in',res = 2000)
plot_grid(base.plot,
          methane.plot,
          annual.plot,
          annual.methane.plot,

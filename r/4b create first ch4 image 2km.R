...
net = which(complete.cases(pca.towers$`2022 list`) & pca.towers$active == 'active' & pca.towers$Start_CO2 < 2022 & pca.towers$methane == 'methane')
...
#save the base here
writeRaster(x = base,filename = './output/methane_2kmv2.tif',overwrite = T)
...
towers  = subset(pca.towers,complete.cases(pca.towers$`2022 list`) & pca.towers$active == 'active' & pca.towers$Start_CO2 < 2022 & pca.towers$methane == 'methane')

png(filename = './figures/methanev2.png',width = 6,height = 6,units = 'in',res = 1000)
ggplot()+theme_map()+

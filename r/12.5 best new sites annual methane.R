...
#First addition ############################################################################################
#ranking of sites
ranks = read.csv(file = './output/reductions/meanreduction_remaining_annual_methane_4_capebounty.csv')
names(ranks)[1] = 'site'
...
tower.data$methane          = ifelse(tower.data$site == "Tura" |
                                       tower.data$site == 'Bonanza Creek, 1987 Burn site near Delta Junction'|
                                       tower.data$site == 'ZOTTO-Forest EC'|
                                       tower.data$site == 'Cape Bounty' |
                                       tower.data$site == 'Spasskaya Pad (experimental forest)',
                                     'methane',tower.data$methane)
...
last = rast('./output/improved_network/annual_methane/improved_annual_methane_4_capebounty.tif')
...
#save the base here
writeRaster(x = base,filename = './output/improved_network/annual_methane/improved_annual_methane_5_spasskaya.tif',overwrite = T)
...
#load in the base
#base = rast('./output/improved_network/annual/improved_annual_methane_5_spasskaya.tif')
...
write.csv(x = bars,file = './output/reductions/meanreduction_remaining_annual_methane_5_spasskaya.csv',row.names = F)

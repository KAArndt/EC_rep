...
tower.data = fread('./data/next_5_sites/base_1.csv')

#First addition ############################################################################################
tower.data$order = seq(1,length(tower.data$MeanTemp)) #important for merging back the tower order

#ranking of sites
ranks = read.csv(file = './output/reductions/meanreduction_remaining_1.csv')
...
write_csv(x = tower.data,'./data/next_5_sites/base_2.csv')
...
orig = rast('./output/improved_network/next_five_sites/improved_base_2km_1.tif')
...
#save the base here
writeRaster(x = base,filename = './output/improved_network/next_five_sites/improved_base_2km_2.tif',overwrite = T)
...
write.csv(x = bars,file = './output/reductions/meanreduction_remaining_2.csv',row.names = F)

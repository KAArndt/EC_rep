...
#ranking of sites
ranks = read.csv(file = './output/reductions/meanreduction_remaining_mean_4_jackpine.csv')
...
tower.data$active  = ifelse(tower.data$site == "Yessey" |
                            ...
                            tower.data$site == 'HJP75 Jack Pine' |
                            tower.data$site == 'Ust Kuyga','active',tower.data$active)
...
#save the base here
writeRaster(x = base,filename = './output/improved_network/next_five_sites/improved_base_2kmv2_mean_5_ust_kuyga.tif',overwrite = T)
...
#saveRDS(object = eucis,file = './euclidean_distance_matrix/ext_eucis_2km_mean_5_ust_kuyga.rds')
#eucis = read_rds(file = './euclidean_distance_matrix/ext_eucis_2km_mean_5_ust_kuyga.rds')
...
#load in the base if starting fresh
base = rast('./output/improved_network/next_five_sites/improved_base_2kmv2_mean_5_ust_kuyga.tif')
...
write.csv(x = bars,file = './output/reductions/meanreduction_remaining_mean_5_ustkuyga.csv',row.names = F)
...
# for (i in 1:length(dif.ag)) {
#   png(filename = paste('./output/remaining_difs/5_ust_kuyga/',extention.towers$site[i],'.png',sep = ''),width = 4,height = 4,units = 'in',res = 100)
#   print(plot_list[[i]])
#   dev.off()
#   progress(value = i,max.value = length(dif.ag))
# }

...
tower.data = fread(file = './data/next_5_sites/annual_methane_1.csv')

tower.data$order = seq(1,length(tower.data$MeanTemp)) #important for merging back the tower order

#First addition ############################################################################################
#ranking of sites
ranks = read.csv(file = './data/reductions/meanreduction_remaining_annual_methane_1.csv')
...
tower.data$annualmethane2024  = ifelse(tower.data$site == name,'annualmethane',tower.data$annualmethane2024)
...
ext = which(tower.data$rank <= 100 & tower.data$annualmethane2024 != 'annualmethane')


#save off
tower.data = tower.data[,-c('rank','country','means','order','type','stats')]
write_csv(x = tower.data,'./data/next_5_sites/annual_methane_2.csv')
...
orig = rast('./output/improved_network/next_five_sites/improved_annual_methane_2km_1.tif')
...
#save the base here
writeRaster(x = base,filename = './output/improved_network/next_five_sites/improved_annual_methane_2km_2.tif',overwrite = T)
...
write.csv(x = bars,file = './data/reductions/meanreduction_remaining_annual_methane_2.csv',row.names = F)

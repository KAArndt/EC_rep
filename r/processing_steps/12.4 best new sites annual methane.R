...
tower.data = fread(file = './data/next_5_sites/annual_methane_3.csv')

tower.data$order = seq(1,length(tower.data$MeanTemp)) #important for merging back the tower order

#First addition ############################################################################################
#ranking of sites
ranks = read.csv(file = './data/reductions/meanreduction_remaining_annual_methane_3.csv')
...
write_csv(x = tower.data,'./data/next_5_sites/annual_methane_4.csv')
...
orig = rast('./output/improved_network/next_five_sites/improved_annual_methane_2km_3.tif')
...
#save the base here
writeRaster(x = base,filename = './output/improved_network/next_five_sites/improved_annual_methane_2km_4.tif',overwrite = T)
...
write.csv(x = bars,file = './data/reductions/meanreduction_remaining_annual_methane_4.csv',row.names = F)

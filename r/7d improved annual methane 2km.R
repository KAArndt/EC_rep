...
{orig = Sys.time() #start the clock for timing the process
  for (i in 1:nrow(euci.net)) {
    impanu.dist[i] = mean(euci.net[i,topn(vec = euci.net[i,],n = num,decreasing = F,hasna = F)])
...
#save the imp here
writeRaster(x = imp,filename = './output/imp_an_methane_2kmv2.tif',overwrite = T)

#######################################################################################
imp = rast('./output/imp_an_methane_2kmv2.tif')
...
old = subset(pca.towers,complete.cases(pca.towers$`2022 list`) & pca.towers$active == 'active' & pca.towers$Start_CO2 < 2022 & pca.towers$Season_Activity == 'All year')

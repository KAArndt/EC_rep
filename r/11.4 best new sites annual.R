...
#First addition ############################################################################################
#ranking of sites
ranks = read.csv(file = './output/reductions/meanreduction_remaining_annual_3_capebounty.csv')
names(ranks)[1] = 'site'
...
#add the #1 site
tower.data$Season_Activity  = ifelse(tower.data$site == "Tura" |
                                     tower.data$site == "Bonanza Creek, 1987 Burn site near Delta Junction" |
                                     tower.data$site == 'Cape Bounty' |
                                     tower.data$site == "Spasskaya Pad (experimental forest)"   ,'All year',tower.data$Season_Activity)
...
#load the last image
last = rast('./output/improved_network/annual/improved_annual_3_capebounty.tif')

dif = last - base
plot(base,range=c(0,4.5))
points(towers,col='red')

plot(dif,range=c(-0.0001,1))
points(towers,col='red')

#save the base here
writeRaster(x = base,filename = './output/improved_network/annual/improved_annual_4_spaskaya.tif',overwrite = T)
#base = rast('./output/improved_network/annual/improved_annual_2_1987bonanza.tif')
...
#load in the base
base = rast('./output/improved_network/annual/improved_annual_4_spaskaya.tif')
...
write.csv(x = bars,file = './output/reductions/meanreduction_remaining_annual_4_spaskaya.csv',row.names = F)

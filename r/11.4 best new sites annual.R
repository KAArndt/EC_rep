...
tower.data = fread(file = './data/next_5_sites/annual_3.csv')

tower.data$order = seq(1,length(tower.data$MeanTemp)) #important for merging back the tower order

#First addition ############################################################################################
#ranking of sites
ranks = read.csv(file = './output/reductions/meanreduction_remaining_annual_3.csv')
...
write_csv(x = tower.data,'./data/next_5_sites/annual_4.csv')
...
orig = rast('./output/improved_network/next_five_sites/improved_annual_2km_3.tif')
...
#save the base here
writeRaster(x = base,filename = './output/improved_network/next_five_sites/improved_annual_2km_4.tif',overwrite = T)
...
#saveRDS(object = eucis,file = './euclidean_distance_matrix/ext_eucis_annual_1_tura.rds')
#eucis = read_rds(file = './euclidean_distance_matrix/ext_eucis_annual_1_tura.rds')
...
write.csv(x = bars,file = './output/reductions/meanreduction_remaining_annual_4.csv',row.names = F)

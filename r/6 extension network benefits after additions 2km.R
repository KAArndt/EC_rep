...
#load in ranked sites
rank = fread(file = './output/ranked sites.csv')
names(rank)[1] = 'site'

pca.towers1 = merge(pca.towers1,rank,by = 'site',all=T)
...
pca.towers1$rank = ifelse(pca.towers1$site == 'Churchill Fen' | 
                                pca.towers1$site == 'Iqaluit' |
                                pca.towers1$site == 'Resolute' |
                                pca.towers1$site == 'Pond Inlet' |
                                pca.towers1$site == 'Kangiqsualujjuaq' |
                                pca.towers1$site == 'CEF cluster' |
                                pca.towers1$site == 'Council (NGEE Arctic)',
                              144,pca.towers1$rank)

#net = which(pca.towers1$Activity == 'active')
#ext = which(pca.towers1$rank < 144/2)

net = subset(pca.towers1,pca.towers1$Activity == 'active')
ext = subset(pca.towers1,pca.towers1$rank < 144/2)

#create some subsets of the euclidean distance tables for easier calculations
euci.net = euci[,c(net$site)]
euci.ext = euci[,c(ext$site)]
...
path = paste('./output/ext/',ext$site,'.tif',sep = '')
#save off rasters
...
#load in the base
base = rast('./output/improve_2km.tif')
...
path = paste('./output/difs/',ext$site,'_dif.tif',sep = '')
#save off rasters
...
bars = data.frame(ext$site)
bars$means = meansv
bars$country = ext$Country[ext]

ext$type = paste(ext$Activity,ext$CH4,ext$Annual_cover,sep = '_')
bars$type = ext$type

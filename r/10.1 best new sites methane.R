...
#First addition ############################################################################################
#ranking of sites
ranks = read.csv(file = './output/meanreduction_methane.csv')
names(ranks)[1] = 'site'
...
#save the base here
writeRaster(x = base,filename = './output/improved_methane_1_tura.tif',overwrite = T)
...
write.csv(x = bars,file = './output/meanreduction_remaining_mean_1_yessey.csv',row.names = F)

...
tower.data = fread(file = './data/next_5_sites/methane_4.csv')

tower.data$order = seq(1,length(tower.data$MeanTemp)) #important for merging back the tower order

#First addition ############################################################################################
#ranking of sites
ranks = read.csv(file = './data/reductions/meanreduction_remaining_methane_4.csv')
...
tower.data$methane.2024 = ifelse(is.na(tower.data$methane.2024),'no',tower.data$methane.2024)
...
write_csv(x = tower.data,'./data/next_5_sites/methane_5.csv')
...
orig = rast('./output/improved_network/next_five_sites/improved_methane_2km_4.tif')
...
#save the base here
writeRaster(x = base,filename = './output/improved_network/next_five_sites/improved_methane_2km_5.tif',overwrite = T)
...
write.csv(x = bars,file = './data/reductions/meanreduction_remaining_methane_5.csv',row.names = F)

...
#First addition ############################################################################################
#ranking of sites
ranks = read.csv(file = './output/reductions/meanreduction_remaining_methane_4_spaskaya.csv')
names(ranks)[1] = 'site'
...
tower.data$methane  = ifelse(tower.data$site == "Tura" |
                             ...
                             tower.data$site == 'Spasskaya Pad (experimental forest)' |
                               tower.data$site == 'NEON Delta Junction (DEJU)','methane',tower.data$methane)
...
#save the base here
writeRaster(x = base,filename = './output/improved_network/methane/improved_methane_5_neondeltajunction.tif',overwrite = T)
...
#load in the base
base = rast('./output/improved_network/methane/improved_methane_5_neondeltajunction.tif')
...
write.csv(x = bars,file = './output/reductions/meanreduction_remaining_methane_5_neondeltajunction.csv',row.names = F)

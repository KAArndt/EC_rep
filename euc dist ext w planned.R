...
png(filename = './output/with.planned/active/base.png',width = 6,height = 6,units = 'in',res = 500)
...
#save the base here
writeRaster(x = base,filename = './output/with.planned/active/base_all_towers.tif',overwrite = T)
...
towers.ext = SpatialPointsDataFrame(coords = pca.towers1[ext,c(1,2)],data = pca.towers1[ext,],proj4string = crs(base))
towers.ext = spTransform(x = towers.ext,CRSobj = proj)
towers.ext.st = as.data.frame(towers.ext)

base.ster = projectRaster(from = base,crs = proj)

png(filename = './output/with.planned/ch4/base.png',width = 6,height = 6,units = 'in',res = 500)
ggplot()+theme_bw()+ggtitle('CH4 Sites')+
  geom_polygon(data = wm,aes(x = long,y=lat,group=group),
               fill = 'gray80',lwd = 0.25)+
  layer_spatial(base.ster$base.dist)+
  geom_point(data = towers.st,aes(x.1,y.1),col='black',fill='green',pch=25,size=2)+
  geom_point(data = towers.ext.st,aes(x.1,y.1),col='green',fill='transparent',pch=25,size=2)+
  scale_fill_gradientn('ED',
                       na.value = 'transparent',
                       colours = pal,
                       #trans = 'log',
...
#save the base here
writeRaster(x = base,filename = './output/with.planned/ch4/base_2_towers.tif',overwrite = T)
...
png(filename = './output/with.planned/ch4/barplot_reduction.png',width = 10,height = 4,units = 'in',res = 2000)
ggplot(data = top)+theme_bw()+ggtitle('Top 50 Percentile Improvements')+
...
png(filename = './output/with.planned/ch4/barplot_reduction_country.png',width = 10,height = 4,units = 'in',res = 2000)
ggplot(data = top)+theme_bw()+ggtitle('Top 50 Percentile Improvements')+
  geom_bar(aes(reorder(sitename, -means*-1),means*-1,fill=country),stat = 'identity')+
  scale_y_continuous(expand = c(0,0),limits = c(0,upper.limit),'Mean ED Reduction')+
  scale_x_discrete('Site')+
...
write.csv(x = bars,file = './output/with.planned/ch4/meanreduction.csv',row.names = F)

#create a path of file names
path = paste('./output/with.planned/ch4/ext/',pca.towers1$sitename[ext],'.tif',sep = '')
#save off rasters
for (i in 1:length(dist.rasts)) {
  writeRaster(x = dist.rasts[[i]],filename = path[i],overwrite=T)
  progress(i,length(dist.rasts))
...
#save off difference maps
path = paste('./output/with.planned/ch4/dif/',pca.towers1$sitename[ext],'_dif.tif',sep = '')
#save off rasters
for (i in 1:length(difs)) {
  writeRaster(x = difs[[i]],filename = path[i],overwrite=T)
  progress(i,length(difs))
...
#plot
path = paste('./output/with.planned/ch4/plots/',pca.towers1$sitename[ext],'_dif.png',sep = '')
pca.towers1$sitename[ext]
...
####################################################################################
#the winter network
net = which(pca.towers1$annual == 'yes' & pca.towers1$active == 'active')
ext = which(pca.towers1$annual != 'yes' & pca.towers1$active == 'active')
...
png(filename = './output/with.planned/annual/base.png',width = 6,height = 6,units = 'in',res = 500)
...
#save the base here
writeRaster(x = base,filename = './output/with.planned/annual/base_2_towers.tif',overwrite = T)
...
png(filename = './output/with.planned/annual/barplot_reduction.png',width = 10,height = 4,units = 'in',res = 2000)
...
write.csv(x = bars,file = './output/with.planned/annual/meanreduction.csv',row.names = F)

#create a path of file names
path = paste('./output/with.planned/annual/ext/',pca.towers1$sitename[ext],'.tif',sep = '')
#save off rasters
for (i in 1:length(dist.rasts)) {
  writeRaster(x = dist.rasts[[i]],filename = path[i],overwrite=T)
  progress(i,length(dist.rasts))
...
#save off difference maps
path = paste('./output/with.planned/annual/dif/',pca.towers1$sitename[ext],'_dif.tif',sep = '')
#save off rasters
for (i in 1:length(difs)) {
  writeRaster(x = difs[[i]],filename = path[i],overwrite=T)
  progress(i,length(difs))
...
#plot
path = paste('./output/with.planned/annual/plots/',pca.towers1$sitename[ext],'_dif.png',sep = '')
...
net = which(pca.towers1$active == 'active' & pca.towers1$ch4 == 'CH4' & pca.towers1$annual == 'yes')
ext = which((pca.towers1$ch4 != 'CH4' | pca.towers1$annual != 'yes') & pca.towers1$active == 'active')
...
png(filename = './output/with.planned/annual ch4/base.png',width = 6,height = 6,units = 'in',res = 500)
...
#save the base here
writeRaster(x = base,filename = './output/with.planned/annual ch4/base_2_towers.tif',overwrite = T)
...
png(filename = './output/with.planned/annual ch4/barplot_reduction.png',width = 10,height = 4,units = 'in',res = 2000)
...
png(filename = './output/with.planned/annual ch4/barplot_reduction_country.png',width = 10,height = 4,units = 'in',res = 2000)
ggplot(data = top)+theme_bw()+ggtitle(expression('Top 25 Percentile Improvements - Annual'~CH[4]))+
  geom_bar(aes(reorder(sitename, -means*-1),means*-1,fill=country),stat = 'identity')+
...
write.csv(x = bars,file = './output/with.planned/annual ch4/meanreduction.csv',row.names = F)

#save off difference maps
path = paste('./output/with.planned/annual ch4/dif/',pca.towers1$sitename[ext],'_dif.tif',sep = '')
#save off rasters
for (i in 1:length(difs)) {
  writeRaster(x = difs[[i]],filename = path[i],overwrite=T)
  progress(i,length(difs))
...
#create a path of file names
path = paste('./output/with.planned/annual ch4/ext/',pca.towers1$sitename[ext],'.tif',sep = '')
#save off rasters
for (i in 1:length(dist.rasts)) {
  writeRaster(x = dist.rasts[[i]],filename = path[i],overwrite=T)
  progress(i,length(dist.rasts))
...
#plot
path = paste('./output/with.planned/annual ch4/plots/',pca.towers1$sitename[ext],'_dif.png',sep = '')
